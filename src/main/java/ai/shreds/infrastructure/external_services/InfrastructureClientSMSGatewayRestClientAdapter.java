 ...
 public class InfrastructureClientSMSGatewayRestClientAdapter implements ApplicationOutputPortSMSGatewayClientPort { 
     ...
     public SharedSMSGatewayResponse sendMessage(SharedSMSGatewayRequest request) { 
         ...
         try { 
             ...
             ResponseEntity<SharedSMSGatewayResponse> responseEntity = 
                 restTemplate.exchange(url, HttpMethod.POST, httpEntity, SharedSMSGatewayResponse.class); 
             ...
             // Assuming the error response has the same structure as SharedSMSGatewayResponse 
             SharedSMSGatewayResponse errorResponse; 
             try { 
                 // Convert responseBody JSON to SharedSMSGatewayResponse 
                 ObjectMapper objectMapper = new ObjectMapper(); 
                 errorResponse = objectMapper.readValue(responseBody, SharedSMSGatewayResponse.class); 
             } catch (IOException ioException) { 
                 // Cannot parse error response body 
                 ...
                 errorResponse.setDetails("Error: " + e.getStatusCode() + " " + e.getStatusText()); 
             } 
