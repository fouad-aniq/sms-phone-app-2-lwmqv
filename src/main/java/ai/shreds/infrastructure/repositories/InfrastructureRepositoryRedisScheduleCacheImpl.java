package ai.shreds.infrastructure.repositories; 
 ...
 public class InfrastructureRepositoryRedisScheduleCacheImpl implements DomainPortScheduleCache { 
     ...
     public List<DomainEntitySchedule> getAllSchedules() { 
         ...
         Cursor<String> cursor = redisTemplate.executeWithStickyConnection(redisConnection -> new ConvertingCursor<>(redisConnection.scan(scanOptions), redisTemplate.getStringSerializer()::deserialize)); 
         try { 
     ...
     public List<DomainEntitySchedule> getSchedulesDueBy(Timestamp time) { 
         ...
         for (DomainEntitySchedule schedule : allSchedules) { 
             if (schedule.getScheduledTime().compareTo(time) <= 0 
                     && schedule.getStatus() == SharedValueScheduleStatus.ACTIVE) { 
